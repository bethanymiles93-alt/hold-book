...
- **The calendar actively filters the list, not just scrolls to an entry within it — corrected 2026-09-01, superseding the "jumps/anchors to that entry, widening the 6-month default window" language from earlier the same week, which described scroll-and-highlight behaviour within an unfiltered list.** Whichever month the calendar is currently showing (its default landing month, reached via prev/next, or via the month picker — all the same case, the list just reflects whatever the calendar displays) filters the list to that month only. Picking a year filters the list to that whole year, across every month. Tapping a specific logged day filters the list down to just that single entry, at the top — not a highlighted card within a longer list. There's no longer a "show all time" toggle — it didn't have an obvious place once the calendar itself became the list's filter control; **open question, not resolved**: whether some other way to see everything unfiltered is still wanted, since the old 6-month-default/all-time-toggle mechanism is gone, not replaced by an equivalent.
...
- Picking a year reveals that year's twelve months as collapsed accordions underneath the calendar (not one long scrollable list) — tap a month to expand just its own entries inline, with one more control to expand or collapse every month in that year at once. This inline accordion browsing stays independent of the list-filter mechanism above — a self-contained way to browse a year's periods without leaving the calendar area.
