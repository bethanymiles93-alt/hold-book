...
1. **Who needs to know?** — choose one or more Circles. Circle setup happens inline here, not as separate onboarding ("+ New Circle" available on this screen; full management lives in Settings).
...
3. **The shared message box** — **first message to a given Circle only:** select an intent (chips: I'm unwell / I need some quiet / Feeling a little overwhelmed / Write my own) or write your own, edit if wanted, then tap **"Save to Library"** — with a short line beneath it, e.g. "This becomes your usual message for [Circle]. Edit it anytime in Library." **Subsequent messages to the same Circle:** the chips don't reappear — the box is pre-filled with the saved default, directly editable, with a small "Change template" link as a low-profile escape hatch. The same "Save" affordance stays available every visit: editing and tapping Save updates the default; editing without saving applies only to that message. **When more than one Circle is selected:** each Circle keeps its own card and its own box — never one shared box for the whole send.
4. **Send.** Fires once: the group message to everyone still included, each excluded-and-not-personalise-tapped person's own instant message, and seeds anyone who tapped Personalise into Conversations' Personalise bucket. This is the point Taking Time actually begins. Each Circle's card collapses in place to the same sent-pill/checkmark treatment already used in Conversations. A plain confirmation line follows: "Sent. You've communicated to everyone who needs to know."
5. **"Want to send anyone something more personal?"** — Yes / Not now. Re-offers the same excluded people from step 2 (whether or not they were already routed to Personalise) with an inline compose box each — not a navigation, an alternative way to reach the same depth right here. Mirrors Conversations' own "an instant message doesn't preclude a fuller follow-up" allowance.
6. **Email out-of-office**, then **"Set your status elsewhere"** — appear here, after Send, not before it. Purely optional toggles.
7. **"Done"** — one explicit, deliberate final tap (not automatic — an untouched toggle is ambiguous about whether someone decided "not now" or just hadn't got to it yet, and this app's own low-capacity design principles argue against an unclear automatic trigger). Activates out-of-office and copies the status text if either was enabled, then moves on to the Transition screen ("You've taken the first step...", unchanged).
