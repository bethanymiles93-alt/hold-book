...
- **New-Circle suggestion chips** ("Friends," "Work," "Book Club") lose their "+" prefix — name only. Visible before typing starts; once the person starts entering their own Circle name, the chips make way for the docked bar itself.
...
- **Dictation:** a mic in the docked bar, using the platform's own on-device speech recognition (iOS's SFSpeechRecognizer, Android's SpeechRecognizer, forced to on-device-only rather than the cloud-accuracy default) — nothing spoken leaves the device for this feature, consistent with Hold's privacy stance elsewhere.
