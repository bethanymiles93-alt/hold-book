...
- **Visual treatment (2026-08-11):** the bar's top edge is a thicker, darker-green border (reusing the app's own Strong/primary green token, no new colour); its fill — and the suggestion chips above it — blend the pale chip green toward the app's neutral border tone (a computed mix of two existing tokens, not an invented one), so the bar reads as a transition into the keyboard rather than a flat block sitting on it, the way Instagram's own message composer does.
