...
4. **Send — per-Circle and repeatable, not a single whole-screen action fired once.** A row of Circle chips sits above the cards, one per selected Circle, using the same interactive selected/sent-this-session chip pattern as Taking Time's "Send an update," Reconnect, and Library's Quick message. Tapping a chip toggles whether that Circle's card is expanded and included in the next Send. Send sends the group message to everyone still included in each currently-selected Circle's card, each excluded-and-not-personalise-tapped person's own instant message, and seeds anyone who tapped Personalise into Conversations' Personalise bucket — for the selected Circles only, not the whole audience at once. Taking Time actually begins at the first successful Send. A just-sent Circle's chip switches to the same sent/checkmark look Conversations and Taking Time's update already use, and its card collapses — but the chip stays tappable: tapping a sent chip re-selects it (its card reopens) so a further or different message can still be sent to that same Circle. Deselecting a sent chip without sending falls back to the sent look, not blank. The screen never auto-completes or locks after one Send — it's fine to send to some Circles now and others later, in any order, rather than requiring everyone at once. A plain confirmation line ("Sent. You've communicated to everyone who needs to know.") appears once anything has been sent, and stays visible alongside the still-usable picker above it.
