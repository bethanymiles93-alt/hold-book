...
**Sequential, group-based redesign (2026-08-11) — supersedes the per-Circle-card version of this list below.** The previous version gave each selected Circle its own independent card, own message, own Send. On-device, this meant a real usability bug (one page-level Send button that only enabled once *every* visible card had a message) got fixed by giving each card its own button — which then exposed the deeper issue: independent per-Circle messages don't match how someone actually thinks about "who am I telling the same thing to." The new model generalises "All" (already special-cased before) to any selected subset: pick some Circles, they share one message and one Send; send it, the selection clears, pick the next group, repeat until Done or every Circle is covered. This is the same flat-selection + shared-message shape Taking Time's "Send an update" already used — Going Quiet just extends it with combination-keyed saved templates (below), which "Send an update" doesn't need since it has no per-Circle default to begin with.

1. **Who needs to know?** — choose one or more Circles, from the same single row of Circle chips used throughout this screen. This row is simultaneously the audience picker AND the send-group picker — whichever Circles are currently selected *are* the current group, sharing the one message box below. Circle setup happens inline here, not as separate onboarding ("+ New Circle" available on this screen; full management lives in Settings). A Circle created here this way stays local to this session — whether to make it a real, persisted Circle is asked later, at Reconnect, once capacity has likely improved, never here (see Reconnect's own section below).
2. **Per selected Circle, its own recipient list** (unaffected by this redesign — still exactly as before): every recipient shown with a **selection circle** — the same visual language as Home's main action circle: solid filled when included in the current group message, white with a border when excluded. Colour is always paired with the fill change, never colour alone.
   - Anyone excluded from the group message gets their own row: a second-level selection circle, their own short instant message (pre-filled from the current group's shared draft, then independently editable), and a **Personalise** link. Tapping Personalise routes them to Conversations instead of sending an instant message — deferred: nothing is seeded into Conversations until Send actually fires.
   - Tapping that same second-level circle again fully removes the person instead — their row collapses to just their name, muted, no box, no link, minimal weight, plus a small checkbox for **bundling into a new Circle** (see below). Tapping the circle once more restores them exactly as they were. One consistent control, never a swipe gesture, never a separate "Remove" link. **Exception: a Circle with only one contact never shows this second-level control at all.**
   - **Splitting a removed person into a new Circle, mid-flow (new, 2026-08-11):** one or more already-removed people — the small checkbox above, can span more than one Circle's list at once — can be spun into a new, provisional Circle via "+ New circle from N selected." Auto-generated placeholder name ("New Circle," numbered if more than one exists this session) — never a naming prompt mid-flow, matching the app's own low-capacity design principles. The new Circle joins the current group's selection immediately. Provisional, not immediately permanent: Reconnect asks whether to make it real (and name it properly) later, and that prompt fires regardless of whether anything was ever sent to it during Going Quiet — the person was already removed from their original Circle the moment the split happened, so if the provisional Circle silently didn't persist when nothing was sent, they'd fall out of every circle with no signal anywhere that they need following up on.
3. **The shared message box — one per current group, not one per Circle.** A Circle-combination previously sent to auto-loads its own saved default, the same no-decision feel a single Circle's own saved default already had. A brand-new combination doesn't auto-load anything — instead it offers **starting points**, one per member Circle that has its own single-Circle saved default ("start from Work's message"), tappable to load that text in; picking none just carries the previous group's edited text forward, rather than resetting to blank. No saved default and no prior text at all (the very first message of the session) falls back to the intent chips (I'm unwell / I need some quiet / Feeling a little overwhelmed / Write my own), same as before. **Saving:** for a single selected Circle, unchanged — the first generic pick auto-persists immediately, an edit only carries forward via explicit "Save to Library." For a combination of two or more Circles, there's no manual save control at all — its own default is written silently, automatically, the moment a message is actually sent for that exact combination (not before, and not on every keystroke).
4. **Send** — one action for the current group, sending: the shared message to everyone still included across every Circle in the group, each excluded-and-not-personalise-tapped person's own instant message, and seeding anyone who tapped Personalise into Conversations. Only a Circle that actually had at least one included recipient in this particular send is marked sent — a Circle whose every member was individually excluded or removed wasn't genuinely reached by it. **After a successful Send, the selection clears back to empty** — that's what makes the "repeat until Done or every Circle is covered" loop work: pick the next group from a clean slate, rather than needing to manually deselect everyone first. A Circle already sent to this session is never locked — reselecting it (alone, or as part of a new combination) is a normal part of the loop, not a special "reselect" mode; its chip simply fills in (dark-green fill, white text, checkmark — the app's one standard sent treatment, not a separate visual for the same idea) to show "sent at least once this session," and un-fills the moment it's back in the current selection, filling in again once sent again. Taking Time actually begins at the first successful Send, from any group. No screen-level "Sent" confirmation line — the per-Circle fill already satisfies "never rely on colour alone," and a blanket line would be inaccurate once sending is partial/repeatable. The restful "you've done what's needed" feeling belongs on the Transition screen instead.
5. **"Want to send personalised messages?"** — Not now / Personalise, reachable once at least one Circle has been sent. Re-offers the excluded people belonging to Circles that have actually been sent so far, whether or not they were already routed to Personalise. **Personalise opens the same rich per-person Send/Edit/Personalise accordion used in Library and Reconnect** (see `04-navigation-architecture.md`'s "Personalise pattern" section) — not a plain inline compose box, and not a navigation away from Going Quiet.
6. **Email out-of-office**, then **"Set your status elsewhere"** — their own step, shown once the Personalise decision above has been made, regardless of which way it went. Never folded into the Personalise yes/no itself. Purely optional toggles.
7. **"Done"** — one explicit, deliberate final tap (not automatic — an untouched toggle is ambiguous about whether someone decided "not now" or just hadn't got to it yet, and this app's own low-capacity design principles argue against an unclear automatic trigger). **Reachable from the very start, before anything at all has been sent** — someone who opens Going Quiet and decides not to send anything this session still needs a way out, not a state that traps them. Before anything's been sent, "Done" sits in the current group's Send position and simply finishes the flow (out-of-office/status toggles aren't offered yet at that point, so there's nothing to activate). Once at least one Circle has been sent, "Done" moves to its position after the Personalise and out-of-office/status steps, activating out-of-office and copying the status text if either was enabled, then moving on to the Transition screen ("You've taken the first step...", unchanged).
