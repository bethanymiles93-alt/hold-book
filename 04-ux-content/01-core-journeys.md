...
1. **Who needs to know?** — choose one or more Circles from the full chip row. Selecting a Circle does two things at once: it joins the *current* group (sharing the message box below with whatever else is selected) and it joins the session's **queue** — every Circle ever selected this session, which only grows, never shrinks (deselecting a Circle for the current message doesn't drop it from the queue). The full row stays visible and scrollable throughout — there's no separate "reopen selection" screen; an unqueued Circle is tapped into the queue the same way any Circle is selected. **Circles currently selected for the message being typed right now float to the front of the row**, recomputed live, so the person can always see who they're currently messaging without scrolling — a deliberate visual behaviour, not incidental. Circle setup happens inline here ("+ New Circle" available on this screen; full management lives in Settings); a Circle created here stays local to this session until Reconnect asks whether to make it real.
