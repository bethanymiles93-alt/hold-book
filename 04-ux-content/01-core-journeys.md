...
- **Retention:** 90 days from creation — see `docs/03-privacy-model.md` (in `hold-app`), "AI memory," including a flagged discrepancy: retention was specified to match "the existing Hold+ trial expiry window," but no such trial exists anywhere in the app (Hold+ isn't purchasable yet), so a flat figure was used instead.
