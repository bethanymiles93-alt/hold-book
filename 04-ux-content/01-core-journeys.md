...
**Add to Going Quiet** (edge case, low-key, not a prominent button): if someone new messages while the user is away, this adds them to Hold's tracked list. **Not yet rebuilt into the same drawer pattern as Send an Update (2026-08-13) — still the older, simpler mechanic** (a contact picker plus a short default message) rather than the fuller "browse Circles, dropdown-arrow reveals a pill row of that Circle's people, matching Going Quiet's own screen" design now specified for it. Sending here is meant to add the new person/Circle to the ongoing tracked audience so they appear correctly in future Update prompts and Reconnect — that part is already true of the existing simple mechanic and carries forward once rebuilt.
