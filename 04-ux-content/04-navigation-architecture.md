...
- **One fixed diameter for every chip in the row — not grow-to-fit per label.** An earlier version of this rule let the circle's diameter grow to fit each label individually (floored at the accessibility minimum, capped short of an oversized blob); revised again after that produced a visibly inconsistent row, different chips reading at different heights. `STANDARD_CHIP_DIAMETER` is one constant: 44pt on iOS, 48pt on Android — the bare platform accessibility minimum itself (44pt HIG / 48dp Material), kept platform-specific by design, not reconciled into one shared number. Set at the floor rather than above it after checking git history: the original pill component this superseded was 38pt for its entire life, below the floor, so the floor itself is the closest legally-compliant match rather than the earlier "+4 comfortable margin" choice. A pill's own horizontal padding is restored to match that original's generosity, kept separate from the (still tight) padding budget the circle-fit check itself uses, so short labels can still plausibly become circles.
...
- **Selection and sent-state are two independent flags, not one** — `isSelected` (part of the current compose action) and `hasSentThisSession` (already sent this session, whatever "this session" scopes to for that flow). Priority: selected wins regardless of sent; else sent shows the softened/desaturated look with no ring; else default. Sent is never cleared by deselecting — only a genuine send changes it — so reselecting an already-sent Circle to send again, then deselecting without sending, correctly returns to the sent look rather than default. Being rolled out screen by screen as each Circle-picker row is migrated onto `AdaptiveCircleChip`; not yet true everywhere a Circle can be selected.
