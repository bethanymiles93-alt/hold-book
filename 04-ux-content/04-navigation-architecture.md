...
- **One fixed diameter for every chip in the row — not grow-to-fit per label.** An earlier version of this rule let the circle's diameter grow to fit each label individually (floored at the accessibility minimum, capped short of an oversized blob); revised again after that produced a visibly inconsistent row, different chips reading at different heights. `STANDARD_CHIP_DIAMETER` is now one constant, deliberately set above the bare accessibility minimum for visual breathing room rather than sitting at the floor: 48pt on iOS, 52dp on Android — kept platform-specific by design (each a flat step above that platform's own guidance, 44pt HIG / 48dp Material), not reconciled into one shared number.
