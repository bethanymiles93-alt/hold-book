...
## Home-screen widget (moon phase + Taking Time status)

**Logged 2026-08-31.** Confirmed as future spec — no widget scaffolding exists anywhere in the app yet (checked directly: no WidgetKit extension target on iOS, no App Widget/Glance setup on Android). This is a net-new native surface on both platforms independently, not an extension of existing work — meaningfully bigger than the moon-phase visual itself, which is now built into Home directly (see `01-decision-log.md`, 2026-08-31). Deliberately deprioritised behind TestFlight-blocking work (IAP/paywall, Settings completion, accessibility audit). Revisit with its own scoping pass, not folded into a general build message, once those are clear.

## Tide indicator (ebb/flow) — distinct from moon phase

**Logged 2026-08-31.** Future spec, not built. Distinct purpose from moon phase: moon phase answers "how long have I been in Taking Time" (elapsed time); tides would answer "where and when am I, right now" — a grounding-to-place-and-moment cue to help with re-entry/coping when returning to messages, not a duration marker. Kept as two separate ideas, not merged, even though they're visually adjacent concepts.

Real open question before this could be built: tide times are local and require a location input, which Hold has no mechanism for today. Should follow the same shape already established for region/safeguarding detection (`06-privacy-security/03-safeguarding.md`) — a coarse, user-confirmed setting (e.g. nearest coastal region, set once, editable in Settings), never live GPS/precise location, no server-side storage. **Not yet resolved:** whether that's a real, worthwhile scope for a single grounding toggle, or whether it's better served by something that needs no location data at all (e.g. time-of-day framing without literal tide data). Needs its own scoping pass before any build decision.
