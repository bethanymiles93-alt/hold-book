...
**Built, though without a working purchase flow yet** — no entitlement/billing system exists in the app. The destination is an honest Hold+ info screen (`app/settings/hold-plus.tsx`) built from this document's real content: what's free, what Hold+ would add, the pricing, and the fair-access commitments below, closing with a plain "not open for purchase yet" note. No fake Subscribe button — building one would invent a purchase flow this book doesn't actually specify. **Re-checked directly against the actual code, 2026-08-31:** `app/settings/hold-plus.tsx` shows the current, correct figures — £4.50 one-time intro, then £3.49/month or £19.99/year; Patterns Report £2.50 (free with Hold+); AI credit pack £2.99. The Founding Member/Standard £17.99/£29.99 split this paragraph previously flagged as still live in the code was not actually present when re-checked — that flag had itself gone stale. The real gap found on re-check was narrower: Patterns Report and AI credit pack pricing were missing from the screen entirely, and were added the same day (`docs/09-decision-log.md` in `hold-app`, 2026-08-31; see also `08-decisions/01-decision-log.md`'s 2026-08-20 pricing row, "Correction, 2026-08-31" addendum).
